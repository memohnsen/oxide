* Oxide - A terminal text editor written in Rust

Oxide is a terminal text editor written in Rust following the Kilo tutorial in C by [[https://viewsourcecode.org/snaptoken/kilo/index.html][Snaptoken]]

Oxide comes with most of the vim motions for manipulating and editing text.
...
** Working with Oxide
You can use Cargo to build and run the project
#+begin_src bash
cargo run
...
When working with the project, 'just run' will open the editor to a test file in the root. If you want to open the welcome screen run 'just welcome' or 'cargo run'
